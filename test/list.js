...
describe('list.js',function(){
    ...
    describe('$ should implement es5 list prototypes',function(){
        ...
        it('implement map',function(){
            ...
            var maps = lis.map(function(el){return el.innerHTML});
        ...
        it('implement filter', function(){
            var lis = $(document.getElementById('container').firstChild.childNodes);
            var maps = lis.filter(function(el){return el.innerHTML -0 <3; });
            expect(maps.length).to.be(2);
        })

        it('implement every', function(){
            var lis = $(document.getElementById('container').firstChild.childNodes);
            expect(lis.every(function(el){return el.tagName.toLowerCase() == 'li'})).to.be(true);
            expect(lis.every(function(el){return el.innerHTML == '1'})).to.be(false);
        })

        it('implement some', function(){
            var lis = $(document.getElementById('container').firstChild.childNodes);
            expect(lis.some(function(el){return el.tagName == 'p'})).to.be(false);
            expect(lis.some(function(el){return el.innerHTML == '1'})).to.be(true);
        })
